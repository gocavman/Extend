...
struct TodaysPlanProvider: TimelineProvider {
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<TodaysPlanEntry>) -> Void) {
        let entry = TodaysPlanEntry(date: Date(), snapshot: readWidgetSnapshot())
        // Refresh at midnight so the day rolls over automatically
...
struct TodaysPlanWidgetView: View {
    ...
    // MARK: Small — icon + "Today's Plan" + item count or rest day
...
// MARK: - Background helper (handles widget rendering context)
